...
savefig(plot(plots_p1 ..., layout=(1,3), size=(3200, 600)),
        string("../out/FD_", gitshorthead(), ".pdf"))	

plots_p2 = []
for j=1:3
    GS = scatter(ρ[j, filter[j, :]], v[j, filter[j,:]], label = "data",
                 title = string("f=v on road ",j), legend=:topright,
                 xlabel="ρ", ylabel="f")
    for k in [1, 2, 5]
        x = LinRange(0, M[j].rhomax, 1000)
        GS = plot!(GS, x, velocity(M[j], x))
    end
    push!(plots_p2, GS)
end

savefig(plot(plots_p2 ..., layout=(3,1), size=(3200, 1800)),
        string("../out/fundamental_diagram_vel_", gitshorthead(), ".pdf"))	
